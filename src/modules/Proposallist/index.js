...
export default class Listui extends BaseComponent {
  ...
  getProposalList = async (skip = 0) => {
    ...
    this.setState({
      proposalsList: updatedList,
      totalProposalsCount: proposals.countData,
    });
  };

  parseProposalList = async (proposals) => {
    if (!window.web3 || !window.web3.currentProvider) return proposals;
    console.log(proposals, "returning proposal");
    let web3;
    web3 = new Web3(window.web3.currentProvider);
    window.ethereum.enable();
    const accounts = await web3.eth.getAccounts();
    if (!accounts || !accounts.length) return proposals;
    const addresses = await getTotalVotingAddress();
    let isWhiteListed = false;
    for (let index = 0; index < addresses.dataList.length; index++) {
      if (
        addresses.dataList[index].address.toLowerCase() ===
        accounts[0].toLowerCase()
      )
        isWhiteListed = true;
    }
    console.log("isWhiteListed ", isWhiteListed, accounts[0]);
    if (!isWhiteListed) return proposals;
    for (let index = 0; index < proposals.length; index++) {
      proposals[index].title = await this.getProposalTitle(
        proposals[index].pollingContract
      );
    }
  ...
  getProposalTitle = async (proposalAddress) => {
    ...
    const createProposalResponse = await contract.methods
      .get_proposal_details_by_proposal_address(proposalAddress)
      .call(tx)
      .catch((err) => {
        console.log(err, "====");
      });
  ...
  handlePageChange = (e) => {
    ...
    this.setState({ activePage: (e - 1) * this.state.limit });
    console.log("current", ((e - 1) * this.state.limit) / 6);
    console.log(Math.ceil(this.state.totalProposalsCount / 6), "total-page");
  };
  ...
  onDateChange = async (e) => {
    ...
    const startTime = moment(e[0].unix * 1000)
      .startOf("day")
      .unix();
    const endTime = moment(e[1].unix * 1000)
      .startOf("day")
      .unix();
    const reqObj = {
      skip: 0,
