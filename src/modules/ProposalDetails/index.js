...
export default class ProposalDetails extends BaseComponent {
  ...
  fetchProposalDetailsFromDB = async (address) => {
    let [error, proposalDetailFromDB] = await Utils.parseResponse(
      ProposalService.getProposalDetail(address, {})
    ).catch((err) => {
  ...
  getProposalDetails = async () => {
    const address = this.props.location.pathname.replace(
      "/proposal-details/",
      ""
    );
    ...
    //     Utils.apiFailureToast("Unable to fetch proposal details");
    // });
    const proposalDetailFromDB = await this.fetchProposalDetailsFromDB(
      address
    ).catch((err) => {
      throw err;
    });
    ...
    const createProposalResponse = await contract.methods
      .get_proposal_details_by_proposal_address(address)
      .call(tx)
      .catch((err) => {
        console.log(err, "====");
      });
    const proposalDetail = { ...proposalDetailFromDB };
    if (!createProposalResponse) return;
    ...
    if (proposalDetail.endDate > Date.now()) {
      ...
      if (isVoted) this.setState({ isButtonClicked: true });
    } else {
  ...
  getSignedUrls = async (documents) => {
    ...
    for (let index = 0; index < documents.length; index++) {
      const url = await getSignedUrlService(documents[index]).catch((err) =>
        console.log(err)
      );
  ...
  isAlreadyVoted = async (proposalAddress) => {
    if (window.xdc) {
      ...
      try {
        ...
        const tx = { from: accounts[0] };
        const yesVotersList = await contract.methods
          .get_yes_voter_list()
          .call(tx)
          .catch((err) => {
            console.log(err, "====");
          });
        const noVotersList = await contract.methods
          .get_no_voter_list()
          .call(tx)
          .catch((err) => {
            console.log(err, "====");
        ...
        if (
          (!yesVotersList || !yesVotersList.length) &&
          (!noVotersList || !noVotersList.length)
        )
          return false;
        ...
        yesVotersList.map((address) => {
          if (accounts[0] === address) isVoted = true;
        });
  ...
  addProposalToDatabase = async (reqData) => {
    const response = await castVotingProposal(reqData).catch((err) =>
      console.log(err)
    );
    const address = this.props.location.pathname.replace(
      "/proposal-details/",
    ...
    const fetchProposalDetailsFromDB = await this.fetchProposalDetailsFromDB(
      address
    );
    const newDetails = {
      ...this.state.proposalDetails,
      ...fetchProposalDetailsFromDB,
    };
